 ...
 * receptacle to select which plug-in to perform operations on
 * @author  Paul Grace
 * @version 1.3.5
 */

public class CalculatorContext extends OpenCOMComponent implements ICalculator, IConnections, ILifeCycle, IUnknown, IMetaInterface {
    
    /**
     * Requires Interface of type IOutput that can be connected to multiple provides interfaces
     * and can be selectively called (i.e. one of the connections is chosen based upon context
     * information.
     */
    public OCM_MultiReceptacleContext<IOutput> m_PSR_IOutput;
    ...
    public CalculatorContext(IUnknown binder) {
        super(binder);
        
        // Initiate receptacles
	m_PSR_IOutput = new OCM_MultiReceptacleContext<IOutput>(IOutput.class);
    ...
    public String display(String message) {
       ...
       /*
        * We will now make a context based call - there are n unknown components connected on this
        * interface. We want to call one interface with contexct: owner="Paul Grace"
        */
       
	m_PSR_IOutput.addContext("Owner", "Paul Grace");    // Attach a context requirement to request
	m_PSR_IOutput.m_pIntf.DisplayMessage(message);   // Invoke operation
       return returnStr;
    }
